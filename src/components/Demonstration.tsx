        ...
        <div className="h-[100vh] snap-center flex justify-center" id="demonstration">
            <div className="w-[1400px] flex justify-between">
                ...
                <div className="items-center flex basis-1/2">
                    <ul className="flex flex-col gap-[20px]">
                        {data.map((item :string) => (
                            <li className="liclass" key={item} data-text={item} onMouseEnter={()=>setThreeDImage(item)}>
                                {item}
                            </li>
                        ))}
                    </ul>
                ...
                <div className='flex basis-1/2'>
                    {threeDImage === 'Front End' ? <FrontEnd/>
                        : threeDImage === 'Web Development' ? <WebDevelopment/>
                            :threeDImage=== 'Where Design' ? <WhereDesign/> :<Life/>}
                </div>
            </div>
        </div>
    );
